...
\end{description}
In the case of supplying only the sentence, the client shall provide a \jsstring{parse\_sentence} field and \jsstring{given\_format} field and may also provide a \jsstring{parser} field and  a \jsstring{wanted\_format} field:
\begin{description}
    \item[\jsstring{parse\_sentence}] A string representing the sentence the server is to parse.
    \item[\jsstring{given\_format}] A string specifying the format the sentence is given as.
    ...
    \item[\jsstring{wanted\_format}] \optional\ A string specifying the format the server should use.
        Servers may ignore this client request if they do not support the requested format.
        If no \jsstring{wanted\_format} is given, servers should use their default format.
\end{description}

\Examples

\lstinputlisting[basicstyle=\footnotesize\ttfamily]{request_parse_sentence.json}
